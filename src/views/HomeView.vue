<template>
  <div class="root_content">
    ...
    <el-dialog custom-class="dialogClass" width="30%" :title="addMsg" v-model="addNum">
  ...
  methods: {
    ...
    init() {
      ...
      request({
        ...
        var html =
          '<p style="font-size: 22px; font-weight: 500; color: black;">你好呀，我是' +
          data.name +
          '</p><p style="font-size: 14px; margin-top: 10px;">' +
          data.desc +
          '</p><p style="font-size: 12px; margin-top: 10px;color:#8e8ea0;">选择下面的模板开始使用吧</p>'
        this.welcome = html
    ...
    login() {
      openNewTab(routerPath.loginPath)
    },
...
.anwser.l2kakEF9 {
  background: rgba(219, 222, 240, 0.4);
}
...
@media screen and (max-width: 1601px) {
  ...
  .welcome_window .welcome_card::-webkit-scrollbar-thumb {
    background: #dde7f7;
    border-radius: 6px;
    -webkit-box-shadow: inset 0 0 3px hsla(0, 0%, 100%, 0.4);
  }
  ...
  .welcome_window .welcome_card .welcome_card_item:hover {
    box-shadow: 0 4px 8px 0 rgba(112, 153, 208, 0.19);
